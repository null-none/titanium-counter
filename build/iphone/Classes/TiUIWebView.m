...
- (WKWebView *)webView
{
  if (_webView == nil) {
    ...
    [controller addUserScript:[self userScriptcounterInjectionForAppEvent]];
...
- (WKUserScript *)userScriptcounterInjectionForAppEvent
{
  if (_pageToken == nil) {
  ...
  NSString *_counter = [NSString stringWithFormat:@"%@%s", @"Ti", "tanium"];
  ...
  NSString *sourceString = [NSString stringWithFormat:source, _counter, _pageToken, baseInjectScript];
...
- (WKUserScript *)userScriptcounterJSEvaluationFromString:(NSString *)string
{
...
- (void)webView:(WKWebView *)webView decidePolicyForNavigationAction:(nonnull WKNavigationAction *)navigationAction decisionHandler:(nonnull void (^)(WKNavigationActionPolicy))decisionHandler
{
  ...
  if ([allowedURLSchemes containsObject:navigationAction.request.URL.scheme]) {
    if ([[UIApplication sharedApplication] canOpenURL:navigationAction.request.URL]) {
      // Event to return url to counter in order to handle OAuth and more
